...
with open(
    os.path.join(os.path.dirname(__file__), "../data/tax_table_2024.json"),
    "r",
    encoding="utf-8",
) as f:
...
class InsuranceCalculator:
    ...
    def calculate_taxes(self, gross_salary, dependents):
        ...
        return {"incomeTax": income_tax, "localIncomeTax": local_income_tax}
    ...
    def get_net_pay(self, gross_salary, dependents):
        ...
        total_deduction = (
            ...
            + insurances["longTermCare"]
            + insurances["employmentInsurance"]
            + taxes["incomeTax"]
            + taxes["localIncomeTax"]
        )
    ...
    def calculate_national_pension(self, monthly_income):
        """국민연금 계산"""
        base_income = max(
            min(monthly_income, self.NATIONAL_PENSION_LIMITS["MAX"]),
            self.NATIONAL_PENSION_LIMITS["MIN"],
        )
    ...
    def calculate_long_term_care(self, health_insurance):
        """장기요양보험 계산"""
        return int(
            health_insurance * self.RATES["LONG_TERM_CARE"] * 0.5
        )  # 직원 부담 50%
    ...
    def calculate_taxable_income(self, salary):
        ...
        total_insurances = (
            self.calculate_national_pension(salary)
        ...
        if income >= 10000000:
            for bracket, info in high_income_brackets.items():
                ...
                if min_val <= income < max_val:
                    ...
                    tax = int(
                        base_tax + (excess_amount * 0.98 * rate) + addition
                    )  # 2% 세액공제 적용
                    return tax
